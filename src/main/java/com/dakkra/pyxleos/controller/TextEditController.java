		...
		if (tem.fileURI != null) {
			...
			if (returnval == JFileChooser.APPROVE_OPTION) {
				System.out.println("Saving "
						+ tem.saveJFC.getSelectedFile().getName() + ".txt");
				try (FileWriter fw = new FileWriter(
						tem.saveJFC.getSelectedFile() + ".txt")) {
					fw.write("" + fullText);
					tem.fileURI = tem.saveJFC.getSelectedFile()
							.getAbsolutePath() + ".txt";
					tem.fileName = tem.saveJFC.getSelectedFile().getName()
							+ ".txt";
					tem.textEditFrame.setTitle(tem.fileName);
					tem.fileReopen.setEnabled(true);
				} catch (IOException e1) {
